...
markersNoQTL1 = markers[,-indexQTL1] # exclude fixed variables
...
bQTL1 = as.matrix(markers[,indexQTL1]) # fixed variables
...
cycles=10 # number of cycles to repeat
...
test = setdiff(1:500, train) #select remanider samples to validate the model

yTRN  = phenoGS[train,] # select 350 lines to train the model
xTRN = markers[train,]
...
yTST  = phenoGS[test,] # select remanider samples to validate the model
xTST = markers[test,]
...
fmGS0 = mixed.solve(yTRN, # only random effects (R)
                    Z=xTRN, 
                    K=NULL,
                    SE= F,
                    return.Hinv = F)

fmGS1 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN1, 
                    Z=x1TRN, 
                    SE= F, 
                    return.Hinv = F)

fmGS2 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN2, 
                    Z=x2TRN, 
                    SE= F, 
                    return.Hinv = F)

fmGS3 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN3, 
                    Z=x3TRN, 
                    SE= F, 
                    return.Hinv = F)

fmGS4 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN4, 
                    Z=x4TRN, 
                    SE= F, 
                    return.Hinv = F)

fmGS5 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN5, 
                    Z=x5TRN, 
                    SE= F, 
                    return.Hinv = F)

fmGS6 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN6, 
                    Z=x6TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS7 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN7, 
                    Z=x7TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS8 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN8, 
                    Z=x8TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS9 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN9, 
                    Z=x9TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS10 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN10, 
                    Z=x10TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS11 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN11, 
                    Z=x11TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS12 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN12, 
                    Z=x12TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS13 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN13, 
                    Z=x13TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS14 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN14, 
                    Z=x14TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS15 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN15, 
                    Z=x15TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS16 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN16, 
                    Z=x16TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS17 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN17, 
                    Z=x17TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS18 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN18, 
                    Z=x18TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS19 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN19, 
                    Z=x19TRN, 
                    SE= F, 
                    return.Hinv = F)
fmGS20 = mixed.solve(yTRN, # QTL fixed  + random (QTL+R)
                    X=bTRN20, 
                    Z=x20TRN, 
                    SE= F, 
                    return.Hinv = F)

yHat0 = xTST %*% fmGS0$u  # Predicted value (R)
yHat1 = (bTST1 %*% fmGS1$beta) + (x1TST %*% fmGS1$u) # Predicted value (QTL + R)
...
accuracy[r,1] = cor(yHat0, yTST, use = "complete" ) # store the correlation of each cycle
...
plot(fmGS1$u, main = "Marker effect") # plot markers effects
abline(v=QTL, col="red")  # add lines in QTL positions
...
predGS1 <- data.frame(yTST, yHat1) # create the data frame input for GP plot

p_GS1 <- ggplot(predGS1, 
               aes(x=yHat1, y=yTST)) +
  geom_point() +
  geom_smooth(method=lm , color="red", fill="#69b3a2", se=TRUE) 
...
coul <- colorRampPalette(brewer.pal(10, "Spectral"))
...
GS with detected QTL (last 11 QTL)
